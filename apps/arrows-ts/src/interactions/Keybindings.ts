...
export type KeyboardEventHandler = (keyboardState: KeyboardState, args:unknown[]) => void

export type KeyboardAction = {
  name: string,
  handler: KeyboardEventHandler,
  bindings: Array<{ altKey?: boolean | 'optional', ctrlKey?: boolean | 'optional', metaKey?: boolean | 'optional', shiftKey?: boolean | 'optional', code: number, codeRange?: {min: number, max: number} }>
}
...
export const ignoreTarget = (ev:Event) => {
  const tagName = (ev.target !== null) ? (ev.target as HTMLElement).tagName : 'NO_ELEMENT'
