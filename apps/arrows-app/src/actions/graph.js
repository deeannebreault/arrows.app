...
export const importNodesAndRelationships = (importedGraph) => {
  return function (dispatch, getState) {
    ...
    let newAnnotationId = nextAvailableId(graph.annotations || []);
    const importedAnnotations = importedGraph.annotations || [];
    importedAnnotations.forEach((oldAnnotation) => {
      const newAnnotation = {
        ...oldAnnotation,
        id: newAnnotationId,
        position: oldAnnotation.position
          ? oldAnnotation.position.translate(vector)
          : undefined,
        points: oldAnnotation.points
          ? oldAnnotation.points.map((p) => p.translate(vector))
          : undefined,
      };
      newAnnotations.push(newAnnotation);
      newAnnotationId = nextId(newAnnotationId);
    });
