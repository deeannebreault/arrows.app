...
const formats = [
  {
    ...
    parse: (plainText) => {
      ...
      const { nodes, relationships, annotations } = graphData.graph;
      const left = Math.min(...nodes.map((node) => node.position.x));
      const top = Math.min(...nodes.map((node) => node.position.y));
      const vector = new Vector(-left, -top);
      const originNodes = nodes.map((node) => translate(node, vector));
      const originAnnotations = annotations.map((annotation) => ({
        ...annotation,
        position: annotation.position
          ? annotation.position.translate(vector)
          : undefined,
        points: annotation.points
          ? annotation.points.map((point) => point.translate(vector))
          : undefined,
      }));
      return {
        nodes: originNodes,
