...
export default class VisualGraph {
    ...
    if (annotation.type === 'TEXT') {
      ...
      if (this.measureTextContext) {
        ...
        const maxWidth = Math.max(
          ...lines.map(
            (line) => this.measureTextContext.measureText(line).width
          )
        );
  ...
  entitiesInBoundingBox(boundingBox) {
    const nodes = this.graph.nodes
      .filter((node) => boundingBox.contains(node.position))
      .map((node) => ({ ...node, entityType: 'node' }));
    const relationships = this.relationshipBundles
      .flatMap((bundle) => bundle.routedRelationships)
  ...
  closestNode(point, hitTest) {
    ...
    this.graph.nodes
      .filter((node) => node.status !== 'combined')
      .forEach((node) => {
  ...
  relationshipAtPoint(point) {
    return this.closestRelationship(
      point,
      (relationship, distance) => distance <= relationshipHitTolerance
    );
  }
    ...
    this.relationshipBundles.forEach((bundle) => {
      bundle.routedRelationships.forEach((routedRelationship) => {
        ...
        if (
          ...
          hitTest(routedRelationship.resolvedRelationship, distance)
        ) {
  ...
  boundingBox() {
    const nodeBoxes = Object.values(this.nodes).map((node) =>
      node.boundingBox()
    );
    const relationshipBoxes = Object.values(this.relationshipBundles).map(
      (bundle) => bundle.boundingBox()
    );
